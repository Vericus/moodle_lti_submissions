...
$context = null;
if (!empty($instanceid)) {
    $psuedolti = $DB->get_record('assign', ['id' => $instanceid], '*', MUST_EXIST);
...
if (!empty($errormsg) || !empty($msg)) {
    $url = new moodle_url('/mod/lti/return.php', ['course' => $courseid]);
    $PAGE->set_url($url);
    ...
    echo $OUTPUT->header();
    if (!empty($psuedolti) && !empty($context)) {
        echo $OUTPUT->heading(format_string($psuedolti->name, true, ['context' => $context]));
    }
}
