...
/**
 * Assign submission lti web service endpoints
 *
...
$consumerkey = lti\get_oauth_key_from_headers(null, [basicoutcomes::SCOPE_BASIC_OUTCOMES]);
if ($consumerkey === false) {
    ...
    if ($toolproxy !== false) {
        $secrets = [$toolproxy->secret];
    } else if (!empty($tool)) {
        $secrets = [$typeconfig['password']];
    } else {
...
switch ($messagetype) {
    case 'replaceResultRequest':
        ...
        $ltiinstance = $DB->get_record('assign', ['id' => $parsed->instanceid]);
    ...
    case 'deleteResultRequest':
        ...
        lti_verify_sourcedid($psuedoltiinstance, $parsed);
        lti_set_session_user($parsed->userid);

        $gradestatus = lti_delete_grade($psuedoltiinstance, $parsed->userid);
