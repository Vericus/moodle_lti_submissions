...
/**
 * The assignsubmission_ltisubmissions\service_base class.
 *
 * @package    assignsubmission_ltisubmissions
 * @since      Moodle 2.8
 *
...
abstract class service_base {
    ...
    public function is_used_in_context($typeid, $courseid) {
        ...
        $ok = $DB->record_exists_sql("SELECT ma.id
            FROM {assign} ma
            JOIN {assign_plugin_config} apc ON apc.assignment = ma.id AND apc.plugin LIKE 'ltisubmissions'
            AND apc.name LIKE 'typeid'
            WHERE ma.course = :course AND apc.value = :typeid ",
            ['course' => $courseid, 'typeid' => $typeid]);
        return $ok || $DB->record_exists('lti_types', ['course' => $courseid, 'id' => $typeid]);
    }
    ...
    public static function get_service_path() {
        ...
        $url = new \moodle_url('/mod/assign/submission/ltisubmissions/services.php');
