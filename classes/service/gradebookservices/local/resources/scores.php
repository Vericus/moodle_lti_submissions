...
class scores extends \ltiservice_gradebookservices\local\resources\scores {
    ...
    public function execute($response) {
        ...
        try {
            ...
            if (!$this->check_tool($typeid, $response->get_request_data(), [$scope])) {
    ...
    private function get_json_for_post_request($response, $body, $item, $contextid) {
        ...
        try {
            // Added for logging the recieved input data.
            $this->log_score_input($item, $score, $score->userId);
            $cmid = $this->get_service()->save_grade_item($item, $score, $score->userId);
            return json_encode(['error' => false,
                'status' => 'Completed Submission import',
                'moodle_grader_url' => "$CFG->wwwroot/mod/assign/view.php?id={$cmid}&action=grader&userid={$score->userId}",
            ]);
        } catch (\Exception $e) {
    ...
    /**
     * Function to log the score object recieved.
     * @param stdClass     $item
     * @param object       $score
     ...
     * @return void
     */
    private function log_score_input($item, $score, $userid) {
        global $DB;
        $loginfo = new \stdClass();
        $loginfo->itemid = $item->id;
        $loginfo->iteminfo = json_encode($item);
