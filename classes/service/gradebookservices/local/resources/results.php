...
class results extends \ltiservice_gradebookservices\local\resources\results {
     ...
     *
     * Overriding it from the mod_lti as we have to use the custom gradebook services instead of the default ones.
     *
     ...
     * @param \assignsubmission_ltisubmission\response $response   The response element needed to add a header.
     *
     * @return string
     */
    private function get_json_for_get_request($itemid, $limitfrom, $limitnum, $useridfilter, $typeid, $response) {
        ...
        if ($useridfilter > 0) {
            $grades = \grade_grade::fetch_all(['itemid' => $itemid, 'userid' => $useridfilter]);
        } else {
            $grades = \grade_grade::fetch_all(['itemid' => $itemid]);
        }
