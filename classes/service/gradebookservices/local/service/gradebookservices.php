...
use assignsubmission_ltisubmissions\service\gradebookservices\local\resources\lineitem;
use assignsubmission_ltisubmissions\service\gradebookservices\local\resources\lineitems;
use assignsubmission_ltisubmissions\service\gradebookservices\local\resources\results;
use assignsubmission_ltisubmissions\service\gradebookservices\local\resources\scores;

use assignsubmission_ltisubmissions\resource_base;
use assignsubmission_ltisubmissions\service_base;
use moodle_url;
...
class gradebookservices extends service_base {
     ...
     * @param string $modassign The id of the assign activity.
     *
     ...
     */
    public function get_launch_parameters($messagetype, $courseid, $user, $typeid, $modassign = null) {
        global $DB, $CFG;
        if (!in_array($messagetype, ['basic-lti-launch-request', 'ContentItemSelectionRequest'])) {
            debugging("Unknown messagetype", DEBUG_DEVELOPER);
        }
        $launchparameters = [];
        ...
        if (!is_null($modassign)) {
            $ltiassignment = $DB->get_record('assign', ['id' => $modassign]);
            if (isset($ltiassignment->allowsubmissionsfromdate) && $ltiassignment->allowsubmissionsfromdate > 0) {
                $launchparameters['allowsubmissions_at_unix'] = $ltiassignment->allowsubmissionsfromdate;
            }
            if (isset($ltiassignment->duedate) && $ltiassignment->duedate > 0) {
                $launchparameters['duedate_at_unix'] = $ltiassignment->duedate;
            }
            if (isset($ltiassignment->cutoffdate) && $ltiassignment->cutoffdate > 0) {
                $launchparameters['cutoff_at_unix'] = $ltiassignment->cutoffdate;
            }
            if (isset($ltiassignment->gradingduedate) && $ltiassignment->gradingduedate > 0) {
                $launchparameters['gradingdue_at_unix'] = $ltiassignment->gradingduedate;
            }
            if (isset($user->id)) {
                $userid = $user->id;
            } else {
    ...
    public function save_grade_item($gradeitem, $score, $userid) {
        global $DB, $CFG;
        $source = 'modassignlti' . $this->get_component_id();
        ...
        if ($gradeitem->is_manual_item()) {
            ...
            if (!$grade = \grade_grade::fetch(['itemid' => $gradeitem->id, 'userid' => $userid])) {
            ...
            $assignsubmission->save_lti_submission($score, $userid);
    ...
    public static function check_lti_1x_id($linkid, $course, $typeid) {
        ...
        if ($ltiactivity) {
            if ($ltiactivity->typeid == 0) {
                ...
                $sql = "SELECT assign.*, apc.value AS typeid
                          FROM {assign} assign
                    JOIN {assign_plugin_config} apc ON apc.assignment = assign.id AND apc.plugin LIKE 'ltisubmissions'
                    AND apc.name LIKE 'typeid' AND apc.subtype LIKE 'assignsubmission'
                         WHERE assign.id = ?
                               AND assign.course = ?
                               AND apc.value = ?";
                return $DB->record_exists_sql($sql, $sqlparams2);
    ...
    public static function update_coupled_gradebookservices(object $ltiinstance,
        ...
        if ($ltiinstance && $ltiinstance->typeid) {
            $gradeitem = $DB->get_record('grade_items', ['itemmodule' => 'assign', 'iteminstance' => $ltiinstance->id]);
            if ($gradeitem) {
    ...
    public static function find_ltiservice_gradebookservice_for_lti($instanceid) {
        ...
        if ($instanceid) {
            $gradeitem = $DB->get_record('grade_items', ['itemmodule' => 'assign', 'iteminstance' => $instanceid]);
            if ($gradeitem) {
